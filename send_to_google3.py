...
class SendToGoogle_onAupFile():
    ...
    def send(self, labeltracks, filename_mp3, in_dir, service):
        """
        ...
        for track in labeltracks:
            ...
            for i in range(flow_number):
                ...
                thread = threading.Thread(target=self.send_in_flow, args=(start, end, values, sound, in_dir, service))
                thread.daemon = True
                thread.start()
        ...
        while self.load_counter < self.number_of_sentence:
            pass
        try:
            shutil.rmtree('.\\tmp\\%d' % in_dir)
        except:
            print("PermissionError: [WinError 5] Отказано в доступе: .\\tmp\\%d" % in_dir)

        return self.sentences_from_google


    def send_in_flow(self, start, end, values, sound, in_dir, service):
        #sound = AudioSegment.from_mp3(filename_mp3)
        for i in range(start, end):
            ...
            text_from_google = requestTranscribe(".\\tmp\\%d\\%d.wav" % (in_dir, i), service)#flac
...
class SendToGoogle():
    ...
    def send(self, filename_mp3, filename_aup, in_dir, min_silence_len, silence_thresh, service):
        ...
        for i in range(flow_number):
            ...
            thread = threading.Thread(target=self.send_in_flow, args=(start, end, in_dir, service))
            thread.daemon = True
            thread.start()
    ...
    def send_in_flow(self, start, end, in_dir, service):
        for i in range(start, end):
            # пропускаем через распознавание речи и получаем список распознаных фраз
            text_from_google = requestTranscribe(".\\tmp\\%d\\%d.wav" % (in_dir, i), service)
