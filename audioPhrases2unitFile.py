...
def audioFilesCompilation(unitNumber, unitFilesNames, unitOutFile, pause = 1.500):
    ...
    for i, file in enumerate(unitFilesNames):
        ...
        rate, err = Popen('soxi -r "./tmp/%s"' % file, shell=True, stdout=PIPE).communicate()
        print file, rate
        if  not int(rate) == 44100:
            subprocess.call('sox "./tmp/%s" "./tmp/_%s" rate -s -a 44100 dither -s' % (file, file), shell=True)
            file = "_%s" % file
        if i == 0:
